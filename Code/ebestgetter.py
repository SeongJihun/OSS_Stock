...
# 로그인하기
#------------------------------------------------------------------------
user=win32com.client.DispatchWithEvents("XA_Session.XASession", XASession)
user.ConnectServer("demo.ebestsec.co.kr", 20001)
user.Login("tjrdns", "tjr2010", 0 , 0, 0)
...
class XAQuery:
    ...
    def OnReceiveData(self, code): #서버로부터 데이터를 수신했을 때 발생한다.;
        print("OnReceiveData", code)
        XAQuery.tr_run_state = 1

    def OnReceiveMessage(self, error, code, message): # 서버로부터 메세지를 수신했을 때 발생한다.
...
class EBest:
    ...
    def __init__(self):
        ...
        self.user = "tjrdns"#config[run_mode]['user']
        self.passwd = "tjr2010" #config[run_mode]['password']
        self.cert_passwd = 0 #config[run_mode]['cert_passwd']
        self.host = "demo.ebestsec.co.kr" #config[run_mode]['host']
        self.port = "20001" #config[run_mode]['port']
        self.account = "55501072185" #config[run_mode]['account']

        self.xa_session_client = win32com.client.DispatchWithEvents("XA_Session.XASession", XASession) #애가 주인공

        self.query_cnt = [] # 시간당 전송 횟수를 제한하기 위한 리스트 만약 200개가 넘으면 time.sleep를 사용해 1초간 정지
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        ...
        """
        time.sleep(1) #10분에 200회 이상 호출할 수 없는 제약사항을 지키기 위한 리스트
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN:
            ...
            self.query_cnt = list(filter(lambda x: (datetime.today() - x).total_seconds() < EBest.LIMIT_SECONDS, self.query_cnt)) #1초가 지나면 600초가 넘지 않는 요소들만 리스트에 담는다
    ...
    def get_current_call_market_price_by_code(self, code=None):
        ...
        in_params = {"shcode": code}
        out_params =["hname", "price", "sign", "change", "diff", "volume",
            "recprice", "avg", "uplmtprice", "dnlmtprice", "jnilvolume", "open", "opentime",
            "high", "hightime", "low", "lowtime", "high52w", "high52wdate", "low52w", "low52wdate",
            "exhratio", "per", "pbrx", "listing", "jkrate", "memedan",
            ...
            "fwdvl", "ftradmdcha", "ftradmddiff", "fwsvl", "ftradmscha", "ftradmsdiff", "vol", "shcode", "value",
            "jvolume", "highyear", "highyeardate", "lowyear", "lowyeardate", "target", "capital", "abscnt", "parprice", "gsmm", "subprice",
            "total", "listdate", "name", "bfsales", "bfoperatingincome", "bfordinaryincome", "bfnetincome", "bfeps",
            "name2", "bfsales2","bfoperatingincome2", "bfordinaryincome2", "bfnetincome2", "bfeps2", "salert", "opert",
            "ordrt", "netrt", "epsrt", "info1", "info2", "info3", "info4", "janginfo", "t_per", "tonghwa", "dval1", "sval1",
            "dval2", "sval2", "dval3", "sval3", "dval4", "sval4", "dval5", "sval5", "davg1", "savg1", "davg2", "savg2", "davg3",
            "savg3", "davg4", "savg4", "davg5", "savg5", "ftradmdval", "ftradmsval", "ftradmdvag", "ftradmsvag", "info5", "spac_gubun",
            "issueprice", "alloc_gubun", "alloc_text", "shterm_text", "svi_uplmtprice", "svi_dnlmtprice", "low_lqdt_gu", "abnormal_rise_gu",
            "lend_text", "ty_text"]
...
"""
get_account_result=user.get_account_info()
print(get_account_result)
"""

"""
#kospi 종목
kospilist=user.get_code_list("KOSPI")
print(len(kospilist))
for i in range(len(kospilist)):
    print(kospilist[i]['단축코드'])
print(kospilist)
"""

## 모든 종목의 단축코드 리스트 이름, 단축코드 리스트 이름은 allnamecode
alllist=user.get_code_list("ALL")

allnamecode=list() #모든 단축코드 리스트

for i in range(len(alllist)):
    ##print(alllist[i]['단축코드'])
    allnamecode.append(alllist[i]['단축코드'])

testlist=list()

"""
for i in range(10):
    outfile = open("주식 결과3.csv", "a", newline='')
    wr = csv.writer(outfile)
    testlist=user.get_current_call_market_price_by_code(allnamecode[i])
    shcode=testlist[0]['단축코드']; hname=testlist[0]['한글명']; price=testlist[0]['현재가']; avg=testlist[0]['가중평균']
    high=testlist[0]['고가']; low=testlist[0]['저가']; PER=testlist[0]['PER']; diff=testlist[0]['등락율']; volume=testlist[0]['누적거래량']
    open=testlist[0]['시가']; pbrx=testlist[0]['PBRX']; listing=testlist[0]['상장주식수(천)']; vol=testlist[0]['회전율']
    total = testlist[0]['시가총액']
    bfsales = testlist[0]['전분기매출액']
    bfoperatingincome = testlist[0]['전분기영업이익']
    bfordinaryincome = testlist[0]['전분기경상이익']
    bfnetincome = testlist[0]['전분기순이익']
    bfeps = testlist[0]['전분기EPS']
    bfsales2 = testlist[0]['전전분기매출액']
    bfoperatingincome2 = testlist[0]['전전분기영업이익']
    bfordinaryincome2 = testlist[0]['전전분기경상이익']
    bfnetincome2 = testlist[0]['전전분기순이익']
    bfeps2 = testlist[0]['전전분기EPS']
    epsrt = testlist[0]['전년대비EPS']
    netrt = testlist[0]['전년대비순이익']
    ordrt = testlist[0]['전년대비경상이익']
    opert = testlist[0]['전년대비영업이익']
    salert = testlist[0]['전년대비매출액']
    if ((float(PER)>=-0.01) & (float(PER)<=0.01)):
        EPS=0
    else:
        EPS = (float(price)) / (float(PER))
    if ((EPS==0) or (netrt==0)):
        PEG=float(PER)/float(netrt)
    else:
        PEG=0
    wr.writerow([shcode, hname, price, avg, high, low, PER, EPS, PEG, netrt])
    s=shcode+', '+hname+', '+price+', '+avg+', '+high+', '+low+', '+PER+', '+str(EPS)+'\n'
    print(i, "번째 주식 : ", s)
    outfile.close()
    
    ## 이건 내 파일 부분
    outfile = open('아예다른.csv', 'a', newline='')
    wr1 = csv.writer(outfile)
    wr1.writerow(
        [shcode, hname, price, avg, high, low, PER, EPS, PEG, netrt, diff, volume, open, pbrx, listing, vol, total,
         bfsales, bfoperatingincome, bfordinaryincome, bfnetincome, bfeps, bfsales2, bfoperatingincome2,
         bfordinaryincome2,
         bfnetincome2, bfeps2, epsrt, netrt, ordrt, opert, salert])
    outfile.close()

# 매일 주식 결과를 CSV로 출력하는 파일
for i in range(len(alllist)):
    outfile = open("주식 결과11.19.csv", "a", newline='')
    wr=csv.writer(outfile)
    testlist = user.get_current_call_market_price_by_code(allnamecode[i])
    shcode = testlist[0]['단축코드'];
    hname = testlist[0]['한글명'];
    price = testlist[0]['현재가'];
    avg = testlist[0]['가중평균']
    high = testlist[0]['고가'];
    low = testlist[0]['저가'];
    PER = testlist[0]['PER'];
    ...
    if ((EPS == 0) or ((float(epsrt) >= -0.01) and (float(epsrt) <= 0.01))):
        PEG = 0
    else:
        PEG = float(PER) / float(epsrt)

    wr.writerow([shcode, hname, price, avg, high, low, PER, EPS, PEG, epsrt, netrt, ordrt, opert, salert])
    #outfile.write(hname, price, avg, high, low, PER, EPS)
    s = shcode + ', ' + hname + ', ' + price + ', ' + avg + ', ' + high + ', ' + low + ', ' + PER + ', ' + str(EPS) + '\n'+ str(PEG) +', '+str(netrt)
    print(i, "번째 주식 : ", s)
    #outfile.write(s)
    time.sleep(3)
...
testlist = user.get_current_call_market_price_by_code("005930"[10])
avg = testlist[0]['가중평균']
print("가중평균은 ", avg)